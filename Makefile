...
DOC_SRC     = doc/NUStheme.dtx
DOC_PDF     = doc/NUStheme.pdf

CTAN_CONTENT = README.md $(INS) $(PACKAGE_SRC) $(DOC_SRC) $(DOC_PDF) $(DEMO_SRC) $(DEMO_PDF)

DESTDIR     ?= $(shell kpsewhich -var-value=TEXMFHOME)
INSTALL_DIR  = $(DESTDIR)/tex/latex/NUS
DOC_DIR      = $(DESTDIR)/doc/latex/NUS
...
ctan: $(CTAN_CONTENT) ctan-version
	@tar --transform "s@\(.*\)@NUS/\1@" -cf NUS-$(shell date "+%Y-%m-%d").tar.gz $(CTAN_CONTENT)
